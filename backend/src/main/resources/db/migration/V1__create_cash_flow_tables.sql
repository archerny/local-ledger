...
-- 创建券商索引
CREATE UNIQUE INDEX idx_broker_name ON brokers(broker_name);
...
-- ============================================
-- 3. 插入初始券商数据（必须在创建出入金记录表之前）
-- ============================================
INSERT INTO brokers (broker_name, country, description, is_active) 
VALUES 
('富途证券 FUTU', 'HK', '港美股互联网券商', FALSE),
('老虎环球 TIGER global', 'NZ', '港美股互联网券商', TRUE),
('盈透证券 IBKR', 'US', '美国本土综合券商', TRUE),
('嘉信证券 charles schwab', 'US', '美国本土综合券商', TRUE),
('第一证券 Firstrade', 'US', '美国本土华人券商', FALSE);
...
-- 创建索引（最小化配置，适合小数据量场景）
-- 1. 日期索引（支持按日期排序和查询）
CREATE INDEX idx_record_date ON cash_flow_records(record_date DESC);

-- 2. 券商外键索引（支持按券商筛选，外键字段建议创建索引）
CREATE INDEX idx_broker_id ON cash_flow_records(broker_id);
